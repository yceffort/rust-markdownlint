...
/// 원본 `showHelp` 본문 (배너 제외). `--diff`, `--stdin-filename`, `server`, `completions` 줄만 추가됐다.
...
- completions     writes a shell completion script (bash, zsh, fish) to standard output (stdout) (rust-markdownlint only)
