...
| `src/tokenizer.rs`, `src/resolve.rs`, `src/construct/text.rs`, `src/construct/partial_data.rs` | `TokenizeState::data_splits` (`DataSplit`/`DataSplitKind`) 추가, `ResolveName::DataSplit` 리졸버(`partial_data::resolve_splits`)를 text 의 맨 마지막에 등록 | micromark 는 data 병합(`resolveAllText`)을 label/attention 리졸버보다 먼저 돌려서 짝 없는 attention 시퀀스와 남은 label 시작이 최상위에서 따로 남는데, markdown-rs 는 반대 순서라 전부 합쳐짐 (`**a [b] c**` 가 data 하나가 되어 MD036 오탐, #186). 리졸버 순서를 바꾸면 이벤트 인덱스를 쓰는 label 주입과 autolink literal 해석이 깨지므로, 전부 합친 뒤 기록해 둔 경계 중 micromark 가 남겼을 것만 되살림. 강조와 링크 라벨 안은 `insideSpan` 리졸버가 다시 합치므로 나누지 않음 |
| `src/construct/label_end.rs` | `resolve()` 가 label 시작이 attention 시퀀스보다 먼저 나왔는지(`label_first`) 계산하고 `mark_as_data()` 가 남은 label 시작을 `data_splits` 에 기록 (각주 시작 `[^` 은 `[` 만) | 위와 동일. micromark 는 리졸버를 처음 성공한 순서로 돌리므로 label 이 먼저면 남은 `[` 가 강조 매치 안에서 합쳐지고, attention 이 먼저면 따로 남음 |
| `src/construct/attention.rs` | `resolve()` 가 짝 없는 시퀀스를 data 로 바꿀 때 `data_splits` 에 기록 | 위와 동일 |
